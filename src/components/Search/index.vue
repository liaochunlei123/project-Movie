    ...
    <div class="search_result">
      ...
      <ul>
        <li v-for="item in moviesList" :key="item.id">
          <div class="img">
            <img :src="item.img | setWH('170.230')" />
          </div>
          <div class="info">
            <p>
              <span>{{item.nm}}</span>
            ...
            <p v-if="item.enm.length<35">{{item.enm}}</p>
            <p v-if="item.enm.length>35">{{item.enm.slice(0,35)+'...'}}</p>
            <p>{{item.cat}}</p>
            <p>{{item.pubDesc}}</p>
          </div>
        </li>
...
export default {
  ...
  watch:{
    message(newVal){
      ...
      // console.log(newVal)
      this.axios.get(
        `/ajax/search?kw=${newVal}&cityId=30&stype=-1`,
        {
